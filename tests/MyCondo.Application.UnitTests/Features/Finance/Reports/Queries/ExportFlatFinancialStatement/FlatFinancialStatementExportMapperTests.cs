...
public class FlatFinancialStatementExportMapperTests
{
    [Fact]
    public void Maps_Metadata_Columns_Rows_And_Totals()
    {
        ...
        FlatFinancialStatementLineDto line = new(
            Guid.NewGuid(), Guid.NewGuid(), new DateOnly(2026, 8, 5), "Service charge", "Invoice", Guid.NewGuid(), "Debit", 500m, 500m);

        FlatFinancialStatementReportDto report = new(
            metadata, Guid.NewGuid(), "A-101", 0m, 500m, 0m, 500m, [line], 1, 100, 1);

        ReportExportDocument document = FlatFinancialStatementExportMapper.ToExportDocument(report);

        document.Title.Should().Be("Flat Financial Statement");
        ...
        document.Totals.Should().Contain(("Period Debit Total", "500.00"));
