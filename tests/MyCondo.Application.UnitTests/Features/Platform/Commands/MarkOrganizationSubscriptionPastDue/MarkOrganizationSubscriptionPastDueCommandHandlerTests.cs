...
namespace MyCondo.Application.UnitTests.Features.Platform.Commands.MarkOrganizationSubscriptionPastDue;

public class MarkOrganizationSubscriptionPastDueCommandHandlerTests
{
    ...
    private MarkOrganizationSubscriptionPastDueCommandHandler CreateHandler() =>
        new(_tenants, _subscriptions, _unitOfWork, Substitute.For<ILogger<MarkOrganizationSubscriptionPastDueCommandHandler>>());

    [Fact]
    public async Task Marks_An_Active_Subscription_Past_Due()
    {
        ...
        await CreateHandler().Handle(new MarkOrganizationSubscriptionPastDueCommand(tenant.Id.Value), CancellationToken.None);
    ...
    [Fact]
    public async Task Throws_When_Subscription_Is_Not_Active()
    {
        Tenant tenant = CreateActiveTenant();
        OrganizationSubscription subscription = CreateSubscription(tenant.Id.Value);
        ...
        Func<Task> act = async () => await CreateHandler().Handle(
            new MarkOrganizationSubscriptionPastDueCommand(tenant.Id.Value), CancellationToken.None);
    ...
    public async Task Throws_NotFound_When_Organization_Does_Not_Exist()
    {
        ...
        Func<Task> act = async () => await CreateHandler().Handle(
            new MarkOrganizationSubscriptionPastDueCommand(organizationId), CancellationToken.None);
