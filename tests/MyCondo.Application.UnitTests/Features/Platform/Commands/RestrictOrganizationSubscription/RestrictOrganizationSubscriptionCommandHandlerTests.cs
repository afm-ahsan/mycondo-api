...
namespace MyCondo.Application.UnitTests.Features.Platform.Commands.RestrictOrganizationSubscription;

public class RestrictOrganizationSubscriptionCommandHandlerTests
{
    ...
    public RestrictOrganizationSubscriptionCommandHandlerTests() => _clock.UtcNow.Returns(NowUtc);
    ...
    private RestrictOrganizationSubscriptionCommandHandler CreateHandler() =>
        new(_tenants, _subscriptions, _unitOfWork, _clock, Substitute.For<ILogger<RestrictOrganizationSubscriptionCommandHandler>>());

    [Fact]
    public async Task Restricts_A_Past_Due_Subscription()
    {
        ...
        await CreateHandler().Handle(new RestrictOrganizationSubscriptionCommand(tenant.Id.Value), CancellationToken.None);

        subscription.Status.Should().Be(OrganizationSubscriptionStatus.Restricted);
        subscription.RestrictedAt.Should().Be(NowUtc);
    ...
    public async Task Throws_When_Subscription_Is_Not_Past_Due()
    {
        ...
        Func<Task> act = async () => await CreateHandler().Handle(
            new RestrictOrganizationSubscriptionCommand(tenant.Id.Value), CancellationToken.None);
    ...
    public async Task Throws_NotFound_When_Organization_Does_Not_Exist()
    {
        ...
        Func<Task> act = async () => await CreateHandler().Handle(
            new RestrictOrganizationSubscriptionCommand(organizationId), CancellationToken.None);
