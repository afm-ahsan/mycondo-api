...
/// <summary>
/// Real RLS enforcement tests for the Slice B tables (security.guest_profiles, security.vehicles,
/// security.access_sessions) — same pattern as <see cref="CrossTenantIsolationTests"/> and
/// <see cref="PropertyResidentsCrossTenantIsolationTests"/>. Requires a Docker daemon. Written and
/// reviewed for correctness but NOT executed in the environment they were authored in (no Docker
/// daemon available there) — run wherever Docker is actually available before trusting them.
/// </summary>
public class SecurityCrossTenantIsolationTests : IClassFixture<MultiTenancyPostgresFixture>
{
    private readonly MultiTenancyPostgresFixture _fixture;

    public SecurityCrossTenantIsolationTests(MultiTenancyPostgresFixture fixture)
    {
        _fixture = fixture;
    }

    [Fact]
    public async Task GuestProfiles_Cross_Tenant_Isolation()
    {
        ...
        {
            dbA.Set<GuestProfile>().Add(
                GuestProfile.Register(tenantA, "Guest A", "01700000001", null, null, DateTimeOffset.UtcNow));
            await dbA.SaveChangesAsync();
        }

        await using (MyCondoDbContext dbB = _fixture.CreateDbContext(tenantB))
        {
            dbB.Set<GuestProfile>().Add(
                GuestProfile.Register(tenantB, "Guest B", "01700000002", null, null, DateTimeOffset.UtcNow));
            await dbB.SaveChangesAsync();
        }

        await using (MyCondoDbContext asTenantA = _fixture.CreateDbContext(tenantA))
        {
            List<GuestProfile> visible = await asTenantA.Set<GuestProfile>().ToListAsync();
            visible.Should().ContainSingle(g => g.FullName == "Guest A");
            visible.Should().NotContain(g => g.FullName == "Guest B");
        }

        await using (MyCondoDbContext noTenant = _fixture.CreateDbContext(tenantId: null))
        {
            List<GuestProfile> visible = await noTenant.Set<GuestProfile>().ToListAsync();
            visible.Should().BeEmpty("a connection with no tenant context set must default-deny, not see every tenant's rows");
        }
    }

    [Fact]
    public async Task Insert_GuestProfile_For_Wrong_Tenant_Is_Rejected_By_Rls()
    {
        Guid tenantA = Guid.NewGuid();
        Guid tenantB = Guid.NewGuid();
        ...
        dbAsTenantB.Set<GuestProfile>().Add(
            GuestProfile.Register(tenantA, "Impersonator", "01700000009", null, null, DateTimeOffset.UtcNow));
    ...
    [Fact]
    public async Task All_Slice_B_Tables_Have_Rls_Enabled_And_Forced()
    {
