...
public sealed class RequestOccupancyRegistrationCorrectionsByManagementCommandHandler(
    IOccupancyRegistrationRepository registrations,
    IOccupancyRegistrationStatusHistoryRepository history,
    IUnitOfWork unitOfWork,
    ICurrentUserProvider currentUser,
    IClock clock,
    ILogger<RequestOccupancyRegistrationCorrectionsByManagementCommandHandler> logger
) : IRequestHandler<RequestOccupancyRegistrationCorrectionsByManagementCommand, OccupancyRegistrationDto>
{
    public async ValueTask<OccupancyRegistrationDto> Handle(
        RequestOccupancyRegistrationCorrectionsByManagementCommand command, CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            "Occupancy registration {OccupancyRegistrationId} sent back for corrections by management, tenant {TenantId}", id, tenantId);
