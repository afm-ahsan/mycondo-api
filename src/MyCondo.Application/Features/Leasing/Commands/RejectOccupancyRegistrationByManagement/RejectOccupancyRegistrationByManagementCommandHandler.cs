...
public sealed class RejectOccupancyRegistrationByManagementCommandHandler(
    IOccupancyRegistrationRepository registrations,
    IOccupancyRegistrationStatusHistoryRepository history,
    IUnitOfWork unitOfWork,
    ICurrentUserProvider currentUser,
    IClock clock,
    ILogger<RejectOccupancyRegistrationByManagementCommandHandler> logger
) : IRequestHandler<RejectOccupancyRegistrationByManagementCommand, OccupancyRegistrationDto>
{
    public async ValueTask<OccupancyRegistrationDto> Handle(
        RejectOccupancyRegistrationByManagementCommand command, CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation(
            "Occupancy registration {OccupancyRegistrationId} rejected by management, tenant {TenantId}", id, tenantId);
