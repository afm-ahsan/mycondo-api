...
public sealed class ApproveServiceProviderAssignmentCommandHandler(
    IServiceProviderAssignmentRepository assignments,
    IUnitOfWork unitOfWork,
    ICurrentUserProvider currentUser,
    ILogger<ApproveServiceProviderAssignmentCommandHandler> logger
) : IRequestHandler<ApproveServiceProviderAssignmentCommand>
{
    public async ValueTask<Unit> Handle(ApproveServiceProviderAssignmentCommand command, CancellationToken cancellationToken)
    {
        ...
        logger.LogInformation("Service provider assignment {AssignmentId} approved by resident for tenant {TenantId}", id, tenantId);
