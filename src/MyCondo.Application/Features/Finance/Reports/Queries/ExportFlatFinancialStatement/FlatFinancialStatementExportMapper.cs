...
public static class FlatFinancialStatementExportMapper
{
    public static ReportExportDocument ToExportDocument(FlatFinancialStatementReportDto report)
    {
        ...
        [
            ...
            ("Period Debit Total", FormatAmount(report.PeriodDebitTotal)),
            ("Period Credit Total", FormatAmount(report.PeriodCreditTotal)),
            ("Closing Balance", FormatAmount(report.ClosingBalance)),
        ];

        return new ReportExportDocument("Flat Financial Statement", metadataLines, columns, rows, totals);
    }
