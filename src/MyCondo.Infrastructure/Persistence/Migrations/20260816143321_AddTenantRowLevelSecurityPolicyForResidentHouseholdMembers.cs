...
/// new <c>residents.household_members</c> table — a new table gets its own new RLS migration rather
/// than editing the immutable clean-baseline <c>AddTenantRowLevelSecurityPolicies</c> migration
/// (mycondo-docs ADR-024; see mycondo-api/.claude/skills/postgresql-rls.md).
/// </summary>
public partial class AddTenantRowLevelSecurityPolicyForResidentHouseholdMembers : Migration
{
    private static readonly (string Schema, string Table)[] TenantScopedTables =
    [
        ("residents", "household_members"),
    ];
